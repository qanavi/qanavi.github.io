...
function handleSignUpPrompt() {
    ...
    const promptForm = document.querySelector("#prompt-form"); // Assuming this is the form element
    promptForm.querySelectorAll("input").forEach(e => { e.disabled = true });
}

window.addEventListener('popstate', function(event) {
    const currentHash = window.location.hash;
    if (currentHash === '#sign-up') {
        handleSignUpPrompt();
    }
});
...
const typed = new Typed('#prompts-sample', {
    strings: ["How to identify data gaps in my dataset?", 
